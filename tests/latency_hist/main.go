	...
	for i, datarate := range datarates {
		...
		for j, pktlen := range pktlens {
			...
			if len(pkts) < trace.GetPacketCount() {
				lossRate = float64((trace.GetPacketCount()-len(pkts)))/float64(trace.GetPacketCount())
			}
			...
			// output some infos
			gofluent10g.Log(gofluent10g.LOG_INFO, "Loss ratio (%d/%d): %.2f", trace.GetPacketCount()-len(pkts), trace.GetPacketCount(), lossRate)
